...
MODULE_DESCRIPTION("XDP RealTek RTL-8169 Gigabit Ethernet driver");
...
static int rtl8169_rx_fill(struct rtl8169_private *tp)
{
	...
	rtl8169_mark_as_last_descriptor(tp->RxDescArray + NUM_RX_DESC - 1);
		/* XDP RX-queue info */
	if (xdp_rxq_info_reg(&tp->xdp_rxq, tp->dev, 0) < 0)
		goto err_out;
					   ...
					   dma_addr_t addr)
{
    ...
    cpu_addr = (u64)phys_to_virt(addr);
...
#if 0
    case XDP_TX:
        result = rtl_xdp_xmit(tp->dev, &xdp);
        //printk(KERN_DEBUG"eno1 xdp tx:%d\n", result);
        if (result == 0)
            tp->xdp_counter.tx++;
    ...
    case XDP_REDIRECT:
        ...
        if (err) {
            printk("xdp do redirect fail err = %d\n", err);
            result = R8169_XDP_DROP;
            tp->xdp_counter.drop++;
            goto xdp_out;
        }
        result = R8169_XDP_REDIR;
        tp->xdp_counter.redirect++;
        goto xdp_out;
    default:
        bpf_warn_invalid_xdp_action(action);
        /* fall through */
    case XDP_ABORTED:
        trace_xdp_exception(tp->dev, xdp_prog, action);
        /* fall through */
    case XDP_DROP:
        tp->xdp_counter.drop++;
        result = R8169_XDP_DROP;
...
process_pkt:
			...
			if (IS_ERR(skb)) {
                ...
                if (xdp_res & (R8169_XDP_TX | R8169_XDP_REDIR)) {
                    xdp_xmit |= xdp_res;
                    dev->stats.rx_packets++;
                    dev->stats.rx_bytes += pkt_size;
                    //printk(KERN_DEBUG"xdp do redirect:11111111\n");
                } else {
                    dev->stats.rx_dropped++;
                }
				goto release_descriptor;
...
static int rtl_xdp(struct net_device *dev, struct netdev_bpf *xdp)
{
